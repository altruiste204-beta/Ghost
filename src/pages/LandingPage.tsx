...
export const LandingPage: React.FC = () => {
  ...
  const handleCreateSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (err.code === 'ALREADY_EXISTS' || err.message?.includes('déjà utilisé') || err.message?.includes('disponible')) {
        ...
        const errorDetail = typeof err === 'string' ? err : (err?.message || err?.toString() || 'Aucun détail technique disponible');
        setError(`Échec de création GHOST (${errorDetail}). Veuillez réessayer dans quelques instants.`);
      }
    } finally {
      setLoading(false);
    }
              ...
              <ScrollReveal direction="right" delay={200}>
                ...
                {activeTab === 'create' && (
                  <form onSubmit={handleCreateSubmit} className="space-y-4">
                    ...
                    {/* Error Notification */}
                    {error && (
                      <div className="p-3 rounded-xl bg-[#FF3B3B]/10 border border-[#FF3B3B]/30 text-xs text-[#FF3B3B] animate-in fade-in duration-200">
