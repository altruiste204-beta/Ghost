...
export const OwnerPage: React.FC = () => {
  ...
  const handleUpdateNumber = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (err.message?.includes('network') || !isNetworkOnline) {
        ...
        setUpdateError(`Échec de la synchronisation : ${errorDetail}`);
      }
    } finally {
  ...
  // Either authenticated via anonymous auth (ownerUid matching user.uid) OR verified credential session
  const isOwner = Boolean(
    (user && meta && meta.ownerUid === user.uid) ||
        ...
        <main className="my-auto">
          <div className="p-4 rounded-2xl bg-[#FF3B3B]/10 border border-[#FF3B3B]/30 mb-5">
            ...
            <p className="text-xs text-[#C0C0C0] leading-relaxed">
              Pour administrer ce GHOST personnel, identifie-toi avec ton numéro de téléphone enregistré.
            </p>
          </div>
